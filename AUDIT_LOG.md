...
- **W-2** `HOUSE_RULES.md` が旧版（R10〜R12 と根拠2行目が無い）。正本は「バイト単位で同一」が約束。`/sync-rules` で更新。 → **2026-09-16 対応済み**（正本と md5 一致）。
...
- **W-2 → 対応済み**: `HOUSE_RULES.md` を正本（Macaron Board / Bonsai Archive と同一内容、
  md5 `71a23d51597c5577b58587880a02c1c8`）で置き換えた。
  旧版に欠けていたのは **R10（外部ファイルへの書き込みは衝突検出→確認→実行）**・
  **R11（永続化した値はUIまで配線されて初めて完成）**・**R12（同じデータを複数プロセスが触るか）** の3ルールと、
  第3部の「外部ファイルへの書き込み」「並行実行」チェックリスト、正本ヘッダ・根拠の2行目。
  ClipMaru でこれから効くのは R10-6（一時ファイル→検証→rename。`safeWrite.ts` が既に準拠）と
  R11（`CurrencyChip.tsx` のような未使用の雛形残骸を残さない）。
- **W-1 / W-3 は未対応**（HEX直書きの解消、`fs:scope` の絞り込み）。
